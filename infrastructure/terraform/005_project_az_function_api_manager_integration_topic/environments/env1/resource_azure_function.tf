...
resource "azurerm_linux_function_app" "az_func_app" {
  ...
  provisioner "local-exec" {
    command = "az functionapp deployment source config-zip -g ${azurerm_linux_function_app.az_func_app_consumer.resource_group_name} -n ${azurerm_linux_function_app.az_func_app_consumer.name} --src ${path.module}/../../../../../app-005/python/app.zip"
  }
...
resource "azurerm_linux_function_app" "az_func_app_consumer" {
  ...
  provisioner "local-exec" {
    command = "az functionapp deployment source config-zip -g ${azurerm_linux_function_app.az_func_app_consumer.resource_group_name} -n ${azurerm_linux_function_app.az_func_app_consumer.name} --src ${path.module}/../../../../../app-005/python-consume/app.zip"
  }
...
resource "azurerm_linux_function_app" "az_func_app_publish" {
  ...
  provisioner "local-exec" {
    command = "az functionapp deployment source config-zip -g ${azurerm_linux_function_app.az_func_app_consumer.resource_group_name} -n ${azurerm_linux_function_app.az_func_app_consumer.name} --src ${path.module}/../../../../../app-005/python-publish/app.zip"
  }
